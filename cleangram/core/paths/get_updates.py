...
class GetUpdates(TelegramPath, abc.ABC, response=Response[List[Update]]):
    """
    ...
    getUpdates, so unwanted updates may be received for a short period of
    time."""

    def adjust(self, bot: Bot, result: List[Update]):
        for r in result:
            r.adjust(bot)
